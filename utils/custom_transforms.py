...
class resize:
    ...
    def __call__(self, sample):
        ...
        if 'mask' in sample.keys():
            sample['mask'] = sample['mask'].resize(self.size, Image.NEAREST)
    ...
    def __call__(self, sample):
        ...
        if np.random.random() < 0.5:
            for key in sample.keys():
                if key in ['image', 'gt','mask', 'contour']:
                    base_size = sample[key].size

                    scale_size = tuple((np.array(base_size) * scale).round().astype(int))
                    if key == 'image':
                        interpolation = Image.BILINEAR
                    else:  # mask, gt, contour
                        interpolation = Image.NEAREST
                    sample[key] = sample[key].resize(scale_size,interpolation)
...
class random_rotate:
    ...
    def __call__(self, sample):
        ...
        if np.random.random() < 0.5:
            for key in sample.keys():
                if key in ['image', 'mask','gt', 'contour']:
                    base_size = sample[key].size
