...
def evaluate(opt, args):
    ...
    snapshots_dir = 'snapshots/UACAPatchNet'  # Directory containing .pth files
    if not os.path.exists(snapshots_dir):
        os.makedirs(snapshots_dir)
    
    pth_files = [f for f in os.listdir(snapshots_dir) if f.endswith('.pth')]
    pth_files.sort()  # Sort to evaluate in order

    Thresholds = np.linspace(1, 0, 256)
    headers = opt.Eval.metrics
    all_results = []  # Store results for all .pth files
    
    if args.verbose is True:
        print('#' * 20, 'Start Evaluation', '#' * 20)

    # Initialize paths and results lists
    pred_root = os.path.join(opt.Eval.pred_root)
    gt_root = os.path.join(opt.Eval.gt_root)
    all_results = []  # Store results for all evaluations
    ...
    # Get the method name from pred_root
    ...
    for i, sample in samples:
        ...
        result.extend(out)
        all_results.append([method, *result])  # Add to all_results directly

        csv = os.path.join(opt.Eval.result_path, 'result.csv')  # Results for all evaluations
    ...
    tab = tabulate(all_results, headers=['model', *headers], floatfmt=".3f")
